...
resource "oci_core_instance" "webserver" {
  ...
  create_vnic_details {
    ...
    hostname_label   = "tfexampleweb${count.index}"
  }
...
resource "oci_core_volume_attachment" "test_block_attach" {
  ...
  instance_id     = oci_core_instance.webserver[floor(count.index / var.num_iscsi_volumes_per_instance)].id
...
resource "oci_core_volume_attachment" "test_block_volume_attach_paravirtualized" {
  ...
  instance_id     = oci_core_instance.webserver[floor(count.index / var.num_paravirtualized_volumes_per_instance)].id
...
resource "oci_core_volume_backup_policy_assignment" "policy" {
  count     = var.num_instances
  asset_id  = oci_core_instance.webserver[count.index].boot_volume_id
...
/*resource "null_resource" "remote-exec" {
  ...
  provisioner "remote-exec" {
    connection {
      agent       = false
      #timeout     = "30m"
      host        = oci_core_instance.webserver[count.index % var.num_instances].public_ip
      user        = "opc"
...
data "oci_core_boot_volume_attachments" "test_boot_volume_attachments" {
  ...
  count               = var.num_instances
  availability_domain = oci_core_instance.webserver[count.index].availability_domain
  compartment_id      = var.compartment_ocid

  instance_id = oci_core_instance.webserver[count.index].id
}*/

data "oci_core_instance_devices" "webserver_devices" {
  count       = var.num_instances
  instance_id = oci_core_instance.webserver[count.index].id
}
...
output "boot_volume_ids" {
  value = [oci_core_instance.webserver.*.boot_volume_id]
}

# Output all the devices for all instances
#output "instance_devices" {
#  value = [data.oci_core_instance_devices.webserver_devices.*.devices]
#}
